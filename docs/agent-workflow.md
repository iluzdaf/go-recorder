...
- After the linked draft PR exists, lock the PR conversation, comment that work continues on the PR, and lock the issue conversation.
- If PR or issue locking fails, keep the linked PR and `needs-plan`, record the lock failure, and stop automation for that issue.
